...
addon_data.config.InitializeVisuals = function()
    ...
    addon_data.config.config_parent_panel = CreateFrame("Frame", "hybarConfigParentPanel", UIParent)
    local panel = addon_data.config.config_parent_panel

    panel:SetSize(1, 1)
    panel.config_panel = addon_data.config.CreateConfigPanel(panel)
    ...
    -- Add the bar panel
    -- panel.config_bar_panel = CreateFrame("Frame", nil, panel)
    -- panel.config_bar_panel:SetSize(1, 1)
    -- panel.config_bar_panel.bar_panel = addon_data.bar.CreateConfigPanel(panel.config_bar_panel)
    -- panel.config_bar_panel.bar_panel:SetPoint('TOPLEFT', 0, 0)
    -- panel.config_bar_panel.bar_panel:SetSize(1, 1)
    -- panel.config_bar_panel.name = "Bar Settings"
    -- panel.config_bar_panel.parent = panel.name
    -- panel.config_bar_panel.default = addon_data.config.OnDefault
    -- InterfaceOptions_AddCategory(panel.config_bar_panel)
...
addon_data.config.CreateConfigPanel = function(parent_panel)
    ...
    -- Title Text
    panel.title_text = addon_data.config.TextFactory(panel, L["Settings"], 20)
    panel.title_text:SetPoint("TOPLEFT", 0, 0)
    panel.title_text:SetTextColor(1, 0.9, 0, 1)
    
    -- Enabled
    panel.is_enabled_checkbox = addon_data.config.CheckBoxFactory(
        "IsEnabledCheckBox",
        panel,
        L["Enabled"],
        L["Uncheck to disable"],
        addon_data.config.IsEnabledCheckBoxOnClick)
